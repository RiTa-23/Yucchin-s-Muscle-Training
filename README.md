...
- **記録と履歴**: 
    - 「今日」のトレーニング詳細と「累計」データを一目で確認。
    - カレンダー形式で過去の全記録（時刻・種目・回数）を振り返り可能。
- **コレクション**: トレーニングを続けることで「ゆっちん」キャラクターを獲得・収集。
- **認証機能**: メールアドレスによる安全なログイン・新規登録（詳細な日本語エラー表示付き）。
- **カスタマイズ設定**: 通知音やキャラクター表示の切り替え、FPS設定など、詳細なユーザー設定が可能。
...
│   │   ├── routers/          # API ルート定義 (auth, users, training等)
...
└── frontend/                 # React Vite フロントエンド
    ...
    │   │   ├── dashboard/    # ホーム画面
    │   │   ├── training/     # トレーニング (Plank等)
    │   │   ├── record/       # 記録・履歴 (RecordPage, HistoryPage)
    │   │   ├── collection/   # キャラクター図鑑
    │   │   └── settings/     # 設定
    │   ├── App.tsx           # ルーティング設定
    │   └── ...
